...
3. **Smallest viable model wins.** Default to the smallest model in every tier (Moonshine Tiny, RapidOCR/PP-OCR mobile models, quantized Whisper-tiny). Bigger models are opt-in, never default.
...
> "Client-side-only converter: anydoc for docs, RapidOCR/Tesseract for images, Moonshine/Whisper for audio, everything in Web Workers, no server ever, smallest model wins by default, handle every edge case explicitly before adding new features."

## Engineering rules for agents

- Detect file type by content (magic bytes / ZIP content-type), never by extension or MIME trust. Route: PDF → pdf-inspector; office formats → anydoc; images → PP-OCR (Tesseract fallback); audio/video → Moonshine/Whisper.
